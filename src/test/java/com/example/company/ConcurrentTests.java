...
public class ConcurrentTests {
        ...
        for (int i = 0; i < N_THREADS; i++) {
            ...
            Runnable runnable =
                () -> {
                    ...
                    SeatHold result = venue
                        .findAndHoldSeats(1, T_NUM + "@email.com");
                    if (result != null) {
                        // If the thread was successful, set the relevant
                        // AtomicBoolean to true
                        boolean anotherThreadSucceeded = singleThreadSucceeded
                            .getAndSet(true);
                        if (anotherThreadSucceeded) {
                            // If another thread was also successful, set the
                            // relevant AtomicBoolean to true
    ...
    public void testOnlyOneThreadSucceedsHoldingLimitedSeats() {
        ...
        // Create 2 threads, each attempting to hold n seats. Only one should
        // succeed

        // Create a venue with 49 seats
        Venue venue = new Venue(7);
        ...
        for (int i = 0; i < 2; i++) {
            ...
            Runnable runnable =
                () -> {
                    ...
                    SeatHold result = venue
                        .findAndHoldSeats(25, T_NUM + "@email.com");
                    if (result != null) {
                        // If the thread was successful, set the relevant
                        // AtomicBoolean to true
                        if (T_NUM == 0) {
