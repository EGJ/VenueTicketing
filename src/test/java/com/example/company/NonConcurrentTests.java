...
public class NonConcurrentTests {
	...
	public void testSeatsChangeAvailabilityWhileSeatHoldIsHeld() {
		...
		assertEquals("Expected seats to become unavailable after SeatHold given", 85, ts.numSeatsAvailable());
		...
		assertEquals("Expected seats to become available after SeatHold expired", 100, ts.numSeatsAvailable());
	}
	...
	public void testSeatingPreferenceGetsBestSeats() {
		...
		SeatHold<Integer> sh1 = v1.findAndHoldSeats(4, "fake@email.com", SeatingPreference.CLOSEST_TO_FRONT);
		SeatHold<Integer> sh2 = v2.findAndHoldSeats(4, "fake@email.com", SeatingPreference.CLOSEST_TO_BACK);
		SeatHold<Integer> sh3 = v3.findAndHoldSeats(4, "fake@email.com", SeatingPreference.CLOSEST_TO_CENTER);
		SeatHold<Integer> sh4 = v4.findAndHoldSeats(4, "fake@email.com", SeatingPreference.CLOSEST_TOGETHER);
		...
		//For closest to back, the top three available seats and the second-from-the-left middle seat should be selected
		//For closest to center, all available middle row seats and the last 2nd row seat should be selected
		//For closest together, all seats in the second from the bottom row should be selected
		Set<Integer> expectedSeats1 = Stream.of(25, 26, 27, 29).collect(Collectors.toSet());
		Set<Integer> expectedSeats2 = Stream.of(0, 2, 3, 7).collect(Collectors.toSet());
		Set<Integer> expectedSeats3 = Stream.of(11, 14, 15, 16).collect(Collectors.toSet());
		Set<Integer> expectedSeats4 = Stream.of(19, 20, 21, 22).collect(Collectors.toSet());

		assertEquals("Unexpected Seating Order for CLOSEST_TO_FRONT", expectedSeats1, reservedSeats1);
		assertEquals("Unexpected Seating Order for CLOSEST_TO_BACK", expectedSeats2, reservedSeats2);
		assertEquals("Unexpected Seating Order for CLOSEST_TO_CENTER", expectedSeats3, reservedSeats3);
		assertEquals("Unexpected Seating Order for CLOSEST_TOGETHER", expectedSeats4, reservedSeats4);
	}
}
